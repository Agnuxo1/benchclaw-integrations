...
async function api<T = any>(path: string, init?: { method?: string; body?: any }): Promise<T> {
  const base = cfg().get<string>("apiBase")!;
...
export function activate(ctx: vscode.ExtensionContext) {
  ctx.subscriptions.push(
    vscode.commands.registerCommand("benchclaw.register", async () => {
      try {
    ...
    vscode.commands.registerCommand("benchclaw.submit", async () => {
      ...
      if (!agentId) return vscode.window.showWarningMessage("Run 'BenchClaw: Register agent' first");
      ...
      try {
        const res = await api<any>("/publish-paper", {
          method: "POST",
          body: {
            agentId,
            title,
    ...
    vscode.commands.registerCommand("benchclaw.leaderboard", async () => {
      try {
        const rows = await api<any[]>("/leaderboard");
